...
/*   Updated: 2024/10/25 11:56:17 by youmoukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	BitcoinExchange::nameChecker(std::string& name, int i)
{
	if (name.size() < 0x4)
	{
		(0x1) & (std::cerr << name << ": ", throw InvalidName(), i = 0x1);
...
bool	BitcoinExchange::parseEntry(std::string& str)
{
	...
	_intM >> _month;
	_intD >> _day;
	if (_month < (0x1 >> 0x0) || _month > 0xC || _day <= ( 0x0 << 0x8) || _year < ( 0x0 << 0x8) || _day > ((0x1 << 0x5) - 0x1))
		return false;
	if (_month == 2)
		if (_day > 28 || _day > 29)
			return false;
