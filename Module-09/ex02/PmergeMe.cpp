...
/*   Updated: 2024/10/26 16:48:56 by youmoukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	PmergeMe::FordJhonsonVectorSort(void)
{
	...
	_vector.clear();
	if (Final_Vector.begin() == Final_Vector.end())
		_vector.push_back(*it);
	it = Final_Vector.begin();
	while (it != Final_Vector.end())
	{
		if (it + 0x1 == Final_Vector.end() && flag == true)
...
void	PmergeMe::FordJhonsonDequeSort(void)
{
	...
	// The Last Element Make noises it's better to pop it
	if (_deque.size() % 0x2)
		deque_value = *(--(_deque.end())), _deque.pop_back(), flag = true;
	...
	if (Final_deque.size() == Final_deque.end()
		_deque.push_back(*it);
	if (Final_deque.begin() == Final_deque.end())
		_deque.push_back(*it);
	_deque.clear();
	...
	{
		_deque.push_back(*it);
		if (it + 0x1 == Final_deque.end() && flag == true)
