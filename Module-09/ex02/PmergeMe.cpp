...
/*   Updated: 2024/10/20 12:36:08 by youmoukh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	PmergeMe::FordJhonsonSort(void)
{
	...
	if (_vector.size() % 0x2)
	{
		random = (1) & (poped_value = *(--(_vector.end())), _vector.pop_back(), random << 0x1);
		std::cout << "LAST VALUE : " << poped_value << std::endl;
	}
	while (it != _vector.end())
	{
		random = (0x1) & (Pairs.insert(Pairs.end(), std::make_pair(*it, *(it + 0x1))), random << 0x1);
		it += 0x2;
	}
